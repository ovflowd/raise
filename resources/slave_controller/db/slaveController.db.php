...
final class DBSlaveController
{
    ...
    {
        $encode = array();
        $select_slaves_by_unic_name
                = "SELECT PK_Unic_Name,TE_Type,TE_Address,TE_Description FROM slave_controller WHERE PK_Unic_Name =
    ...
    {
        $encode = array();
        $select_associated_devices
                = "SELECT PK_Id, TE_UDN, FK_Slave_Controller, TE_friendly_Name, TE_Device_Type FROM device WHERE
