...
final class DBActionController
{
    ...
    public function select_action_by_id($id, $connection)
    {
        $encode = array();
        $select_action_by_id
                = "SELECT PK_Id,TE_Name,FK_Service FROM action WHERE PK_Id =
		'{$id}' AND BO_Deleted = 0;";
    ...
    {
        $encode = array();
        $select_action_device
                = "SELECT FK_device FROM service WHERE
    ...
    private function select_action_name($id, $connection)
    {
        $encode = array();
        $select_action_by_id
                = "SELECT TE_Name FROM action WHERE PK_Id =
		'{$id}' AND BO_Deleted = 0;";
        $result = $connection->query($select_action_by_id);
