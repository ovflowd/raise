...
class RequestTester
{
	...
	public function registerServices($token)
	{
		...
		$body = json_encode(array(
					"services" => array(array('name'=>'temperature','parameters'=>array('temp'=>'vagabond') , 'return_type' => 'float'),array('name'=>'pressure','parameters'=>array('press'=>'vagabond') , 'return_type' => 'float')),
						"timestamp" => round(microtime(true) *1000),
						'tokenId' => $token
						));
	...
	public function testAutoRegister()
	{
			...
			$dados = [ 'token' => $token,  array(array('service_id' => $service[1]->service_id , 'values' => array(array('press'=>(float) 4.5))))];
			echo "<br><br>";
